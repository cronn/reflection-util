...
public class ClassUtilsTest {
  ...
  void testGetRealClass() {
    ...
    assertThat(ClassUtils.getRealClass(createJdkProxy(SomeTestInterface.class)))
        .isSameAs(SomeTestInterface.class);
    assertThat(ClassUtils.getRealClass(createJdkProxy(SomeTestInterface.class).getClass()))
        .isSameAs(SomeTestInterface.class);
    assertThat(ClassUtils.getRealClass(createByteBuddyProxy(new TestEntity())))
        .isSameAs(TestEntity.class);
    assertThat(ClassUtils.getRealClass(createJavassistProxy(new TestEntity())))
        .isSameAs(TestEntity.class);
    assertThat(
            ClassUtils.getRealClass(createByteBuddyProxy(createJavassistProxy(new TestEntity()))))
        .isSameAs(TestEntity.class);
    assertThat(ClassUtils.getRealClass(Long.valueOf(16))).isSameAs(Long.class);

    HibernateProxyTestUtil.runWithHibernateProxy(
        personProxy -> {
          assertThat(ClassUtils.getRealClass(personProxy))
              .isSameAs(HibernateProxyTestUtil.Person.class);
        });
        ...
        .isThrownBy(
            () ->
                ClassUtils.getRealClass(
                    createJdkProxy(SomeTestInterface.class, BaseInterface.class)))
    ...
    assertThatExceptionOfType(IllegalArgumentException.class)
        ...
        .withMessage(
            "The provided object is already a class: class de.cronn.reflection.util.testclasses.TestEntity."
  ...
  void testCreateNewInstanceLikeProtectedConstructor() {
    ...
    assertThatExceptionOfType(ReflectionRuntimeException.class)
        ...
        .withMessage(
            "Failed to construct an instance of " + EntityProtectedNoDefaultConstructor.class);
  }

  @Test
  public void testGetVoidMethod() {
    Method voidMethod =
        ClassUtils.getVoidMethod(ClassUtilsTest.class, ClassUtilsTest::testGetVoidMethod);
  ...
  public void testGetVoidMethodName() {
    String voidMethodName =
        ClassUtils.getVoidMethodName(ClassUtilsTest.class, ClassUtilsTest::testGetVoidMethod);
    assertThat(voidMethodName).isEqualTo("testGetVoidMethod");

    String methodName =
        ClassUtils.getVoidMethodName(new ClassUtilsTest(), ClassUtilsTest::testGetVoidMethod);
    assertThat(methodName).isEqualTo("testGetVoidMethod");

    assertThat(
            ClassUtils.getVoidMethodName(SomeTestInterface.class, SomeTestInterface::doOtherWork))
        .isEqualTo("doOtherWork");
  }
  ...
  void testGetMethodName() {
    assertThat(ClassUtils.getMethodName(TestEntity.class, TestEntity::getNumber))
        .isEqualTo("getNumber");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::getNumber))
        .isEqualTo("getNumber");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::getSomePath))
        .isEqualTo("getSomePath");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::getString))
        .isEqualTo("getString");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::doNothing))
        .isEqualTo("doNothing");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::clear)).isEqualTo("clear");
    assertThat(ClassUtils.getMethodName(new TestEntity(), TestEntity::countSomeList))
        .isEqualTo("countSomeList");
    assertThat(ClassUtils.getMethodName(TestEntity.class, TestEntity::countSomeList))
        .isEqualTo("countSomeList");
    assertThat(ClassUtils.getMethodName(SomeClass.class, SomeClass::doOtherWork))
        .isEqualTo("doOtherWork");
    assertThat(ClassUtils.getMethodName(new SomeClass(), SomeClass::doOtherWork))
        .isEqualTo("doOtherWork");
  }

  @Test
  void testGetMethod() {
    assertThat(ClassUtils.getMethod(TestEntity.class, TestEntity::getNumber).getName())
        .isEqualTo("getNumber");
    assertThat(ClassUtils.getMethod(new TestEntity(), TestEntity::getNumber).getName())
        .isEqualTo("getNumber");
    assertThat(ClassUtils.getMethod(new TestEntity(), TestEntity::getSomePath).getName())
        .isEqualTo("getSomePath");
    assertThat(ClassUtils.getMethod(new TestEntity(), TestEntity::doNothing).getName())
        .isEqualTo("doNothing");
    assertThat(ClassUtils.getMethod(new TestEntity(), TestEntity::countSomeList).getName())
        .isEqualTo("countSomeList");
    assertThat(ClassUtils.getMethod(TestEntity.class, TestEntity::countSomeList).getName())
        .isEqualTo("countSomeList");
  }

  @Test
  void testGetMethod_lambdaWithException() {
    PropertyGetter<TestEntity> getter =
        bean -> {
          throw new IllegalStateException("some exception");
        };
  ...
  @Test
  void
      testHasMethodWithSameSignature_happyPath_shouldMatchMethodSignature_whenReturnTypeAndNameAndParametersAreEqual()
          throws Exception {
    Method targetMethod = findMethod(SomeClass.class, "doWork", int.class);

    assertThat(ClassUtils.hasMethodWithSameSignature(SOME_TEST_INTERFACE_CLASS, targetMethod))
        .isTrue();
  }

  @Test
  void
      testHasMethodWithSameSignature_shouldNotMatchMethodSignature_whenReturnTypeIsDifferentThanHappyPath()
          throws Exception {
  ...
  @Test
  void
      testHasMethodWithSameSignature_shouldNotMatchMethodSignature_whenNameIsDifferentThanHappyPath()
          throws Exception {
  ...
  @Test
  void
      testHasMethodWithSameSignature_shouldNotMatchMethodSignature_whenParametersAreDifferentThanHappyPath()
          throws Exception {
  ...
  void testIsFromPackage() {
    ...
    assertThat(ClassUtils.isFromPackage(SomeClass.class, "de.cronn.reflection.util.testclasses"))
        .isTrue();
  }
  ...
  void testFindDeclaredMethodsByArgumentTypes() {
    assertThat(
            ClassUtils.findMethodsByArgumentTypes(
                FindMethodByArgumentTypesTestCaseClass.class, String.class, Integer.class))
        .hasSize(2);
    assertThat(
            ClassUtils.findMethodsByArgumentTypes(
                FindMethodByArgumentTypesTestCaseSubclass.class, String.class, Integer.class))
        .hasSize(3);
  }
  ...
  void testHaveSameSignature() throws Exception {
    ...
    Method doWorkWithOneParameterFromInterface =
        SomeTestInterface.class.getMethod("doWork", int.class);
    Method doWorkWithTwoParameters = SomeClass.class.getMethod("doWork", int.class, int.class);
    assertThat(ClassUtils.haveSameSignature(doWorkWithOneParameter, doWorkWithTwoParameters))
        .isFalse();
    assertThat(
            ClassUtils.haveSameSignature(
                doWorkWithOneParameter, doWorkWithOneParameterFromInterface))
        .isTrue();
  }

  @Test
  void testGetAllDeclaredMethods() {
    assertThat(withoutJacocoMethods(ClassUtils.getAllDeclaredMethods(BaseInterface.class)))
        .hasSize(1);
  ...
  void testGetAllDeclaredMethodSignatures() {
    Set<MethodSignature> methodsOfSomeClass =
        withoutJacocoMethodSignatures(ClassUtils.getAllDeclaredMethodSignatures(SomeClass.class));
    withoutJacocoMethodSignatures(methodsOfSomeClass);
    assertThat(mapToString(methodsOfSomeClass))
        .containsExactly(
            "void doOtherWork()",
            "void doWork(int)",
            "void doWork(int, int)",
    ...
    Set<MethodSignature> methodsOfDerivedClass =
        withoutJacocoMethodSignatures(
            ClassUtils.getAllDeclaredMethodSignatures(DerivedClass.class));
    assertThat(mapToString(methodsOfDerivedClass))
        .containsExactly(
    ...
    assertThat(
            withoutJacocoMethodSignatures(
                ClassUtils.getAllDeclaredMethodSignatures(SomeClass.class)))
        .hasSize(4);
  }

  @Test
  void testCreateInstance_AccessibleFlagIsRestored() throws Exception {
    Constructor<EntityProtectedConstructor> constructor =
        EntityProtectedConstructor.class.getDeclaredConstructor();
  ...
  void testFindAnnotation() throws Exception {
    ...
    Method setFieldWithAnnotationOnSetter =
        TestEntity.class.getMethod("setFieldWithAnnotationOnSetter", String.class);
    ...
    for (Class<?> clazz :
        Arrays.asList(
            ClassWithInheritedDefaultMethods.class, SubclassOfClassWithDefaultMethods.class)) {
  ...
  private static Set<MethodSignature> withoutJacocoMethodSignatures(
  ...
  private static List<String> mapToString(Collection<MethodSignature> methodSignatures) {
    return methodSignatures.stream().map(MethodSignature::toString).collect(Collectors.toList());
  }
  ...
  private static Object createJdkProxy(Class<?>... interfaces) {
    return Proxy.newProxyInstance(
        ClassUtilsTest.class.getClassLoader(), interfaces, (p, method, args) -> null);
  }

  private static <T> T createByteBuddyProxy(T object) {
    try (DynamicType.Unloaded<T> unloadedType =
        new ByteBuddy().subclass(ClassUtils.getRealClass(object)).make()) {
      Class<? extends T> proxyClass =
          unloadedType.load(ClassUtilsTest.class.getClassLoader()).getLoaded();
      return ClassUtils.createNewInstance(proxyClass);
    }
  }

  private static Method findMethod(Class<?> aClass, String name, Class<?>... parameterTypes)
      throws Exception {
