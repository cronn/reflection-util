...
public class DeepImmutableListTest {
  ...
  void testCreateImmutableProxy() {
    List<TestEntity> immutableProxy =
        ImmutableProxy.create(Collections.singletonList(new TestEntity(123)));
  ...
  void testWriteMethodsAreRejected() {
    ...
    testEntity.setSomeList(Arrays.asList(new OtherTestEntity("a"), new OtherTestEntity("b")));
  ...
  void testReadOnlyMethods() {
    ...
    assertThat(immutableList.subList(0, 1).get(0).getName())
        .isSameAs(immutableList.get(0).getName());

    assertThat(immutableList.subList(1, 2)).hasSize(1);
    assertThat(immutableList.subList(1, 2).get(0).getName())
        .isSameAs(immutableList.get(1).getName());
    ...
    assertThat(immutableList.listIterator(1).next().getName())
        .isEqualTo(immutableList.get(1).getName());
